...
void Matrix::WriteBin(ofstream &fileBin)
{
	fileBin.write((char*)&m, sizeof(m));
	fileBin.write((char*)&n, sizeof(n));
	for (int i = 0; i < m; i++)
        for (int j = 0; j < n; j++)
			fileBin.write((char*)&Element(i, j), sizeof(Element(i, j)));

}

Matrix Matrix::ReadBin(ifstream &fileBin)
{
    int col, row;
	fileBin.read((char*)&col, sizeof(m));
	fileBin.read((char*)&row, sizeof(n));
    Matrix Temp(col, row);		
    for (int i = 0; i < col; i++)
        for (int j = 0; j < row; j++)
			fileBin.read((char*)&Temp.Element(i, j), sizeof(Temp.Element(i, j)));
    return Temp;
}
...
double Matrix::VectAngle(Matrix& rv)
{
	if ((n != 1 && m != 1) || (rv.n != 1 && rv.m != 1))
		throw DimentionError("\nSome dimentions error\nOne or both are not vectors\n");
    cout << Scalar(rv) / (Norm() * rv.Norm()) << endl;
    return acos(Scalar(rv) / (Norm() * rv.Norm())) * 180.0 / 3.14159265;
}
